<script lang="ts">
    import Icon from '$lib/components/ui/icon';
    import Image from './(assets)/sites.svg';
    import Badge from './badge.svelte';
    import { format } from 'date-fns';

    const startDate = new Date('2025-08-29');
    const endDate = new Date('2025-09-12');

    const hackathonDates = `${format(startDate, 'MMM d')} - ${format(endDate, 'MMM d')}`;
</script>

<div
    class="mx-auto flex w-full items-center gap-4 bg-[url(data:image/svg+xml;base64,)] py-4 md:h-[76px]"
>
    <div class="md:basis-1/3">
        <Badge class="ml-0">Sites Hackathon is live <span class="text-accent">/</span> {hackathonDates}</Badge>
    </div>
    <a href="https://hackathon.appwrite.network" target="_blank" class="md:basis-1/3">
        <img class="mx-auto mt-2 hidden max-w-[75px] md:block" src={Image} alt="Appwrite" />
    </a>
    <div class="mr-0 ml-auto justify-self-end text-right">
        <a href="https://hackathon.appwrite.network" target="_blank" class="text-primary group mr-0 ml-auto flex items-center gap-2 text-right"
            >Go to hackathon <Icon
                class="transition group-hover:-translate-x-0.5"
                name="arrow-right"
            /></a
        >
    </div>
</div>
