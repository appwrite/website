...
Each presence is attached to a user and carries a `status` string, an optional `metadata` JSON object for richer context, and an `expiresAt` timestamp that controls automatic cleanup. Presences are written by the user's own session or a server SDK, and read by any client with the right [permissions](/docs/advanced/security/permissions).
