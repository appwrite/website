    ...
    try {
        ...
        if (process.env.NODE_ENV === 'production' && githubEmail) {
            try {
                ...
                console.error('Error sending to user list:', error);
        ...
        if (!githubAccount?.total) {
            const countQuery = await appwriteInitServer.databases.listDocuments(
                APPWRITE_DB_INIT_ID,
                APPWRITE_COL_INIT_ID,
                [Query.orderDesc('id'), Query.limit(1)]
            );
        ...
        console.error('Error in getTicketDocByUser:', error);
        throw new Error('Failed to process user ticket data');
    }
};
