<script lang="ts">
  ...
  let selected = withPrevious(
    users.flatMap((u) => u.devices.map((d) => d.checked)),
  );
  ...
  withRaf(() => {
    animationProgress = animationProgress.map((curr, i) => {
      ...
      const wasSelected =
        $prevSelected[lines[i].from] && $prevSelected[lines[i].to];
      let reverse = false;
...
<Step title="Step 2: Target">
  <div class="wrapper" bind:this={wrapper}>
    ...
    >
      ...
      {#each devices as device, i}
        ...
        <foreignObject x={textPos[0]} y={textPos[1]} width="360" height="64">
          <span
            class="web-eyebrow web-u-color-text-primary"
      ...
      <defs>
        {#each lines as line, i}
          ...
          >
            ...
            <stop
              offset="{selectionPercent}%"
              stop-color="hsla(178, 54%, 69%, 1)"
            />
            <stop
              offset="{selectionPercent}%"
              stop-color="hsla(0, 0%, 100%, 0.08)"
            />
    ...
    <div class="users-modal">
      ...
      <Accordion>
        <ul class="accordion-items">
          {#each users as user, i}
            {@const selectedDevices = user.devices.filter(
              (d) => d.checked,
            ).length}
      ...
      <div class="u-margin-block-start-20 flex">
      ...
      <div class="u-main-end flex items-center gap-4">
        <span style="font-size: 14px; color: #6C6C71;"
          >{users.filter((u) => !!u.devices.some((d) => d.checked)).length} Users
          selected</span
        >
