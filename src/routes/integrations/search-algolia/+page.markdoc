...
Send a POST (HTTP) Request to the function URL to index the Appwrite table in Algolia.

```bash
curl -XPOST https://DEPLOYED_FUNCTION_DOMAIN
```

In case you have a large table of data and your function executions timeout, you can head to the **Settings** tab on the **Functions** page and increase the function timeout limit from **15 seconds** to **900 seconds** (maximum).
