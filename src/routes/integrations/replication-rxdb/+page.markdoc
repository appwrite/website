...
isPartner: true
isNew: true
cover: /images/integrations/replication-rxdb/cover.avif
category: databases
product: 
    avatar: '/images/integrations/avatars/rxdb.avif'
    vendor: Upstash
...
platform: 
    ...
    - 'Cloud'
images: 
    - /images/integrations/replication-rxdb/cover.avif
---
...
Click on the **Columns** tab and add the following columns (schema used for demo purposes):
...
> **Note:** The `deleted` column is necessary to add because RxDB only soft deletes to prevent data loss in offline scenarios (no hard deletion of data occurs).

Then, head to the **Settings** tab of your table, scroll down to the **Permissions** section, and the following:

| Role | Create | Read | Update | Delete |
| --- | --- | --- | --- | --- |
| Any | Yes | Yes | Yes | Yes |

> **Note:** While RxDB does not allow you to manually configure permissions for each row, if your app uses Appwrite Auth and row-level permissions are enabled for your table, the Appwrite SDK will automatically assign read and write permissions to the logged-in user for each row created.
...
> **Note:** Please ensure that the names of your RxDB database and collection match the IDs of your Appwrite database and table.
> 
...
const replicationState = replicateAppwrite({
    ...
    collectionId: process.env.APPWRITE_TABLE_ID,
