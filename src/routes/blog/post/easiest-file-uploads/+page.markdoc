...
date: 2024-08-16
cover: /images/blog/easiest-file-uploads/cover.png
timeToRead: 6
...
```js
const fileUrl = storage.getFileView('profile-photos', fileId);
...
For user-owned files, set `fileSecurity` to `true` on the bucket and pass per-file permissions when creating the file:
...
```js
await storage.deleteFile('profile-photos', fileId);
```
