...
const execution = await functions.createExecution(
    '<FUNCTION_ID>',
    JSON.stringify({ userId: '<USER_ID>', action: 'password_reset' }),
    false,     // async
    '/',       // path
    'POST'     // method
);
...
This pattern keeps privileged operations as server-side Functions with their own access policies, rather than exposing them as direct database operations from the admin panel.
