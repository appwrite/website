...
<NavigationMenu.Root class={className} {...rest}>
    <NavigationMenu.List class="flex items-center gap-3">
        {#each navItems as item}
            <NavigationMenu.Item
                class="rounded-md transition-colors hover:bg-white/6 hover:text-white"
            >
                {#if item.menu}
                    {@const Submenu = item.menu}
                    <NavigationMenu.Trigger class="block px-2 py-1"
                        >{item.label}</NavigationMenu.Trigger
                    >
                    <Submenu />
                {:else}
                    <NavigationMenu.Link href={item.href} class="block px-2 py-1"
