...
function getParameters(
  ...
  const schemaJson = requestBody?.content["application/json"]
...
export async function getService(
  ...
  const isAndroidKotlin =
    platform === Platform.ClientAndroidKotlin ||
    platform === Platform.ServerKotlin;
  const isAndroid = isAndroidJava || isAndroidKotlin;
  const isAndroidServer =
    platform === Platform.ServerJava || platform === Platform.ServerKotlin;
  ...
  for (const { method, value, url } of iterateAllMethods(api, service)) {
    ...
    const responses: SDKMethod["responses"] = Object.entries(
      operation.responses ?? {},
    ).map((tuple) => {
  ...
  const example = properties.reduce((carry, currentValue) => {
    ...
    if (property.type === "array") {
      ...
      if (property.items && "anyOf" in property.items) {
        ...
        const firstSchema = (property.items as unknown as AppwriteSchemaObject)
          ?.anyOf?.[0];
    ...
    if (property.type === "object") {
      if (property.items?.oneOf) {
        ...
        const schema = getSchema(
          getIdFromReference(
            property.items.oneOf[0] as OpenAPIV3.ReferenceObject,
          ),
          api,
        );
        return {
