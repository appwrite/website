...
<script lang="ts">
  ...
  const productsScales = products.map((_, idx) => {
    ...
    return [
      animScale[0] + step * idx,
      animScale[0] + step * (idx + 1),
    ] as Scale;
  });
    ...
    {#if scrollInfo.percentage < 0.075}
      ...
      >
        {#if scrollInfo.percentage > -0.1}
          ...
          <p
            class="web-description web-max-w-[700px] mx-auto"
            transition:fly={{
              y: 16,
              delay: 400,
            }}
          >
            Build secure and scalable applications with less code. Add
            authentication, databases, storage, and more using Appwrite's
